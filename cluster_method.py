...
per_iter = 8 # 单词迭代生成的特征样本，总样本数为 n * per_iter
...
for i in range(n):
  ...
  with torch.no_grad():
    if g_name == 'stylegan2':
      out = generator.get_features_pre(sample_z) # stylegan2
...
#训练kmeans模型, 19.038225412368774
train_kmeans = True # True
if train_kmeans == True:
  kmean_save_path = './checkpoint/clustering_segmentation/dataset_%s_kmeans_k_clusters_%d_feature_size_%d_samples_%d_feature_layer_%s_trunc_%s.pkl'\
  %(dataset,k_clusters,feature_size,n*per_iter,feature_layer,truncation_alpha)
  kmeans_model = KMeans(n_clusters=k_clusters, random_state=3, verbose=2)
  time_start=time.time()
  print('training start')
  kmeans_model.fit(arr)
  time_end=time.time()
  print('train_kmeans time cost: %s'%str(time_end-time_start))
  joblib.dump(kmeans_model,kmean_save_path)

# comparison GMM, time cost: 9.52090859413147
train_gmm = True
if train_gmm == True:
  from sklearn.mixture import GaussianMixture
  gmm_save_path = './checkpoint/clustering_segmentation/dataset_%s_gmm_k_clusters_%d_feature_size_%d_samples_%d_feature_layer_%s_trunc_%s.pkl'\
  %(dataset,k_clusters,feature_size,n*per_iter,feature_layer,truncation_alpha)
  ...
  GMM_model.fit(arr)
  time_end=time.time()
  print('train_gmm time cost: %s'%str(time_end-time_start))
  joblib.dump(GMM_model,gmm_save_path)

# comparison DBSCAN, 1.4015254974365234
train_DBSCAN = True
if train_DBSCAN == True:
  from sklearn.cluster import DBSCAN
  eps = 0.5
  min_samples=8
  DBSCAN_save_path = './checkpoint/clustering_segmentation/dataset_%s_DBSCAN-eps%s-minS%s_k_clusters_%d_feature_size_%d_samples_%d_feature_layer_%s_trunc_%s.pkl'\
  %(dataset,eps,min_samples,k_clusters,feature_size,n*per_iter,feature_layer,truncation_alpha)
  DBSCAN_model = DBSCAN(eps = eps, min_samples=min_samples)
  time_start=time.time()
  print('training start')
  DBSCAN_model.fit(arr)
  time_end=time.time()
  print('train_DBSCAN time cost: %s'%str(time_end-time_start))
  joblib.dump(DBSCAN_model,DBSCAN_save_path)

# comparison SpectralClustering, 1.4440338611602783
train_SC = True
if train_SC == True:
  from sklearn.cluster import SpectralClustering
  SC_save_path = './checkpoint/clustering_segmentation/dataset_%s_SC_k_clusters_%d_feature_size_%d_samples_%d_feature_layer_%s_trunc_%s.pkl'\
  %(dataset,k_clusters,feature_size,n*per_iter,feature_layer,truncation_alpha)
  SC_model = SpectralClustering(n_clusters=k_clusters, random_state=3, affinity='nearest_neighbors')
  time_start=time.time()
  print('training start')
  DBSCAN_model.fit(arr)
  time_end=time.time()
  print('train_SC time cost: %s'%str(time_end-time_start))
  joblib.dump(SC_model,SC_save_path)

...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, out: torch.Tensor) -> torch.Tensor:
        ...
        # similarity matrix
        sim = torch.mm(out, out.t().contiguous()) # [k*batch_size, k*batch_size], k个方向 (即feature差) 变成一个矩阵
        ...
        # mask for pairs
        mask = torch.zeros((n_samples, n_samples), device=sim.device).bool() # [k*batch_size, k*batch_size]
        ...
        pos = sim.masked_select(mask * diag_mask).view(n_samples, -1) #正的输出结点
        # mt = mask * diag_mask
        # print(diag_mask.shape)
        neg = sim.masked_select(~mask).view(n_samples, -1) # #负的输出结点
        # print(sim)
        # print(mask)
        # print(pos)
        # print(neg)

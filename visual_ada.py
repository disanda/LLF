...
conf_path =  './utils/inference_config/visual_config_ffhq_ada.yaml' # 
cfg = yaml.load(open(conf_path), Loader=yaml.FullLoader)
cfg = OmegaConf.create(cfg)
dirs = cfg.n_dirs
save_imgs = cfg.save_single_imgs
...
import legacy
generator = legacy.load_model(cfg.generator_path, device)
...
class Visualizer:
    def __init__(
        ...
        if isinstance(n_samples, int):
            ...
            try:
                print(f"Loading input file {n_samples} for real image")
                self.samples = torch.load(n_samples).to(self.device) # [-1,18,512] from pytorch
            except:
                self.samples = torch.from_numpy(np.load(n_samples)).to(self.device) # [-1,512] from np
        if self.samples.ndim < 3: # [-1,512] -> [-1,18,512]
            self.samples = self.samples.unsqueeze(1).repeat(1,self.n_latent,1).to(self.device)
...
#             self.dirs = np.random.choice(self.model.k, n_dirs, replace=False)
    ...
    def visualize(self) -> float:
        #Generates images from the trained modelReturns: (float) accuracy (on a 0 to 1 scale)
        # Progress bar
        ...
        def _edit(z, alpha, ks): #Apply Directions # [1, 18, 512] - > [k, 18, 512]
            self.model.alpha = alpha
            ws = []
            for j in range(self.n_latent):
                zs = []
                for i,k in enumerate(ks):
                    zk = self.model.forward_single(z[:,j,...], k=k) # [1,512]
                    zs.append(zk)# [1,512]-> [k,512] 
                zs = torch.cat(zs, dim=0).expand(1,-1,-1) # [k,512] -> [1,k,512]
                ws.append(zs)
            ws = torch.cat(ws, dim=0).transpose(1,0) # [1,k,512] -> [18,k,512] -> [k,18,512]
            return ws

        def _generate(zs, z_orig=None):  # Manipulate only asked layers, # [k , 18, 512], [1,18,512]
            if self.feed_layers is not None and z_orig is not None:
                n_latent = self.n_latent
                zs_layers = []
                for i in range(n_latent):
                    if i in self.feed_layers:
                        zs_layers.append(zs[:,i,...].expand(1,-1,-1)) # [k,18,512] -> [k,512] -> [1,k,512]
                    else:
                        zs_layers.append(z_orig[:,i,...].expand(1,zs.shape[0],-1)) #[1,18,512] -> [1,512] -> [1,k,512]
                zs = torch.cat(zs_layers, dim=0).transpose(1,0)  # [1,k,512] -> [18, k, 512] - > [k,18,512], do not stack!
            _, images = self.generator.synthesis(zs, noise_mode='const', force_fp32=True)
            return self.image_transform(images)

        # Loop
        with torch.no_grad():
            for i in range(self.samples.shape[0]): #[2]
                # Take a single sample
                z = self.samples[i : i + 1, ...] # [1,18,512]

                # Get original image
                orj_img = _generate(z) # [b, c, h, w]

                if save_imgs:
                    for j in self.dirs:#img_path+'/%d'%j
                ...
                z_orig = z
                for di, alpha in enumerate(reversed(self.neg_alphas)):

                    z = _edit(z_orig, alpha, ks=self.dirs) # [1,18,512] -> [k,18, 512]
                    one_neg_img = _generate(z, z_orig) # [k , 18, 512], [1,18,512] - > [k,3, h,w]
                    images.append(one_neg_img.detach().cpu())

                    if save_imgs:
                        for j in self.dirs:
                            single_img = one_neg_img[j-self.dirs[0]]
                            torchvision.utils.save_image(single_img.view([1] + list(single_img.shape))*0.5+0.5, img_path+'/%d/'%(j)+"dir-%d_id-%d_neg-%d_seed-%d-layers%s_%s.png"%(j,i,alpha,self.seed,self.feed_layers,self.name), nrow=1)

                # Reverse images
                images = list(reversed(images))

                # Reset z and alpha
                for di, alpha in enumerate(self.pos_alphas):
                    z = _edit(z_orig, alpha, ks=self.dirs)
                    one_pos_img = _generate(z, z_orig)
                    images.append(one_pos_img.detach().cpu())

                    if save_imgs:
                        for j in self.dirs:
                            single_img = one_pos_img[j-self.dirs[0]]
                            torchvision.utils.save_image(single_img.view([1] + list(single_img.shape))*0.5+0.5, img_path+'/%d/'%(j)+"dir-%d_id-%d_pos-%d_seed-%d-layers%s_%s.png"%(j,i,alpha,self.seed,self.feed_layers,self.name), nrow=1)

                #  Prepare final image
                images = torch.stack(images, dim=0)
                images = images.transpose(1, 0)
                col_orj_img = orj_img.repeat((images.size(0), 1, 1, 1)).cpu()  # .unsqueeze(1)

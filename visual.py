...
conf_path =  './utils/visual_config.yaml'
...
class Visualizer:
    def __init__(
        ...
        if self.samples.ndim < 3: # [-1,512] -> [-1,18,512]
            self.samples = self.samples.unsqueeze(1).repeat(1,self.generator.n_latent(),1).to(self.device)
...
#             self.dirs = np.random.choice(self.model.k, n_dirs, replace=False)
    ...
    def visualize(self) -> float:
        ...
        def _edit(z, alpha, ks): #Apply Directions # [1, 18, 512] - > [k, 18, 512]
            ...
            ws = []
            for j in range(self.generator.n_latent()):
                zs = []

...
g_name = 'stylegan2-ada' # 'stylegan2' or 'stylegan2-ada'
k_clusters = 5 # 4*4 : 3  8*8 : 5  16*16 : 7  32*32 : 9 
feature_size = 16 #pow(2,5), ada should feature_size*2
...
kmean_load_path = "./checkpoint/kmeans_segmentation/kmeans_k_clusters_5_feature_size_16_samples_256_feature_layer_convs.3.pkl"
...
sample_z = torch.from_numpy(np.random.RandomState(int(seed)).randn(per_iter, generator.z_dim)).to(device)
...
save_path = f'./outputs/segments/feature_layer={feature_layer}_k_clusters={k_clusters}/'
...
for img_id in range(per_iter): # features.shape[0]
  ...
  plt.savefig(save_path+f'img_id_%d.png'%(img_id),bbox_inches='tight',pad_inches = -0.1)
  plt.show()
  ...
  plt.savefig(save_path+f'img_id_%d_clusters%d_alpha_%.2f.png'%(img_id,k_clusters,truncation_alpha),bbox_inches='tight',pad_inches = -0.1)
  plt.show()
